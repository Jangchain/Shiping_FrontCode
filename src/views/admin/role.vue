  ...
  <div class="app-container">
    <el-form inline :model="searchForm" class="search-form" size="mini">
      ...
      <el-form-item label="角色编码">
        <el-input v-model="searchForm.code" placeholder="输入角色编码"></el-input>
      </el-form-item>
    ...
    <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <router-link :to="{ path: '/admin/role/edit?id=' + scope.row.id + '&type=detail' }">
            <el-button type="text" size="mini">查看</el-button>
          </router-link>
          <template v-if="scope.row.type !== 0">
            <router-link :to="{ path: '/admin/role/edit?id=' + scope.row.id +'&type=edit' }">
...
<script>
import { getRoleList, deleteRole, deleteRolesAll } from "@/api/admin";
export default {
  data() {
    return {
      tableData: [],
  ...
  methods: {
    ...
    create() {
      this.$router.push({
        path: "/admin/role/edit?type=create"
      });
    },
