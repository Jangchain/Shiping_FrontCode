...
SM2Cipher.prototype = {
    ...
    Encrypt: function(pubKey, plaintext) {
        ...
        var c3 = new Array(32);
        this.Dofinal(c3);
        var hexString = this.GetHexString64(c1.getX().toBigInteger().toRadix(16)) + this.GetHexString64(c1.getY().toBigInteger().toRadix(16)) + this.GetHex(data).toString() + this.GetHexString64(this.GetHex(c3).toString());
        if (this.cipherMode == SM2CipherMode.C1C3C2) {
            hexString = this.GetHexString64(c1.getX().toBigInteger().toRadix(16)) + this.GetHexString64(c1.getY().toBigInteger().toRadix(16)) + this.GetHexString64(this.GetHex(c3).toString()) + this.GetHex(data).toString()
        }
        return hexString
    },
    ...
    Decrypt: function(privateKey, ciphertext) {
        ...
        if (this.cipherMode == SM2CipherMode.C1C3C2) {
            ...
            encrypData = hexString.substr(hexString.length - 64)
        }
        ...
        this.Dofinal(c3_);
        var isDecrypt = this.GetHexString64(this.GetHex(c3_).toString()) == c3;
        if (isDecrypt) {
